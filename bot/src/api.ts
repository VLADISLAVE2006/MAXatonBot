...
export const api = {
    ...
    reminders: {
        getPending: () =>
            _fetch<{ registration_id: number; user_id: number; event_id: number; event_title: string; event_date: number; reminder_type: "day_before" | "hour_before" }[]>(
    ...
    events: {
        ...
        getOrganizerArchivedEvents: (token: string) =>
            _fetch<{ id: number; title: string; description: string; date: number; format: string; type: string; max_slots: number | null; registered_count: number; image_url: string }[]>("/api/organizer/events/archived", {
                method: "GET",
                token,
            }),
